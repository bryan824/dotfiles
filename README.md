...
- Agent harness deployment is intentionally separate from chezmoi: use
  `bun run kirin-pi apply` from the Kirin checkout for local testing, or
  `bunx github:bryan824/kirin-pi apply` explicitly for the remote version.
  `chezmoi apply` never replaces the currently deployed harness.
